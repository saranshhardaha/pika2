...
export default function ImageGrid({ images, onSelect, selectedImages }: ImageGridProps) {
  ...
  return (
    <>
      <div className="max-w-[2000px] mx-auto px-4 py-6 bg-[#0a0a0a]">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-1 sm:gap-2">
              {group.images.map((image, index) => (
                ...
                >
                  ...
                  <input
                    type="checkbox"
                    className="absolute top-2 left-2 h-5 w-5 rounded bg-neutral-800 border-gray-600 z-20"
      ...
      {/* Mobile bottom bar */}
      <div className="fixed bottom-0 left-0 right-0 bg-neutral-900 border-t border-gray-800 p-4 md:hidden">
        <div className="flex justify-between items-center max-w-[2000px] mx-auto">
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              <Download size={18} />
              <span>{selectedImages.size > 0 ? 'Download Selected' : 'Download All'}</span>
            </button>
          </div>
        </div>
      </div>
